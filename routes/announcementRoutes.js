...
const express = require('express');
const router = express.Router();
const Announcement = require('../models/Announcement.js'); // Added missing import
const authMiddleware = require('../middleware/authMiddleware');
const jwt = require("jsonwebtoken");
...
router.get('/get-announcement', authMiddleware, async (req, res) => {
    try {
      ...
      if (status === "sent") {
        // User looking at announcements they created and sent
        filter = { senderId: userId, status: "sent" };
      } else if (status === "draft") {
        // User looking at their drafts
        filter = { senderId: userId, status: "draft" };
      } else if (status === "inbox") {
        // User looking at announcements sent to their class
        if (!std || !className) {
          return res.status(400).json({ error: "Missing std or className for inbox filter" });
        }
        filter = {
