...
router.post('/add-announcement', authMiddleware, async (req, res) => {
  try {
    const { std, className, publishTo, title, description, status } = req.body; // Changed subject/message to title/description

    const newAnnouncement = new Announcement({
      std,
      className,
      publishTo,
      title,        // Matches MongoDB 'title'
      description,  // Matches MongoDB 'description'
      status,
